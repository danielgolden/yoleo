...
export default defineComponent({
  ...
  methods: {
    ...
    handleDragEndEvent(e: VueDraggableEvent) {
      const activeItemIsBeingDragged =
        e.oldIndex === store.currentWordListIndex;
...
<template>
  <section
    v-if="primaryNavigationActive"
    :class="{ 'primary-navigation': true, open: store.mainMenuOpen }"
  >
    <button
      class="menu-button"
      @click="store.mainMenuOpen = !store.mainMenuOpen"
    >
      <!-- prettier-ignore -->
...
.omni-input {
  ...
  font-size: 14px;
  box-shadow: 0px 3px 5px var(--color-box-shadow),
...
@media (prefers-color-scheme: dark) {
  ...
  .omni-input:focus {
    box-shadow: 0px 3px 5px transparent, 0px 1px 2px transparent,
      0 0 0 2px var(--color-bg-input-border-enabled);
    outline: none;
  }
}
