  ...
  computed: {
    settingsMenuItems() {
      return [
        ...
        { label: 'Lowercase', icon: 'lowercase', onClick: () => this.$emit('caseChanged', 'lower'), active:  store.gameSettings.case === 'lower' },
        { label: 'Sentence case', icon: 'sentenceCase', onClick: () => this.$emit('caseChanged', 'sentence'), active: store.gameSettings.case === 'sentence' },
        { label: 'Uppercase', icon: 'uppercase', onClick: () => this.$emit('caseChanged', 'upper'), active: store.gameSettings.case === 'upper' },
        { separator: true },
        { label: 'Sound', icon: store.gameSettings.audio ? 'unmute' : 'mute', onClick: () => store.gameSettings.audio = !store.gameSettings.audio, active: store.gameSettings.audio },
      ]
    }
  }
})
