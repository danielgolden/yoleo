...
<template>
  <Menu
    :class="{ 'menu': true, 'settings-menu': settingsMenu, [classes as string]: classes}"
  ...
  >
    <MenuItems as="ul" static>
      <MenuItem
        ...
        @mouseover="
          (e) => {
            if (item.destructive) handleHover(e);
          }
        "
        @mouseleave="
          (e) => {
      ...
      >
        <Icon
          v-if="item.icon"
          :opacity="settingsMenu && '.5'"
          :name="item.icon"
          :color="getIconColor(item.destructive)"
        />
        {{ item.label }}
        <Icon
          v-if="item.active"
